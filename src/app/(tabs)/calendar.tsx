...
try {
const outfits = db.getAllSync<OutfitItem>('SELECT id, name FROM outfits ORDER BY id DESC') as OutfitItem[];
...
return (
  <View style={styles.dateWrapper}>
    ...
    >
      <Text style={[styles.dayName, { color: theme.subtext }, isSelected && styles.textWhite]}>
        {getDayName(d)}
      </Text>
      <Text style={[styles.dayNumber, { color: theme.text }, isSelected && styles.textWhite]}>
        {getDayNumber(d)}
      </Text>
    </TouchableOpacity>
  </View>
...
<View style={[styles.header, { backgroundColor: theme.card, borderBottomColor: theme.border }]}>
    ...
    <Text style={[styles.subtitle, { color: theme.subtext }]}>Track what you wear every day.</Text>
    ...
    <View style={styles.controlsRow}>
      ...
      <TouchableOpacity style={[styles.jumpBtn, { backgroundColor: theme.primary }]} onPress={() => setJumpModalVisible(true)}>
  ...
  <View style={styles.content}>
    ...
    {loggedOutfit ? (
      <View style={[styles.loggedCard, { backgroundColor: theme.card, shadowColor: theme.primary }]}>
        ...
        <View style={styles.actionsRow}>
          <TouchableOpacity style={[styles.changeBtn, { backgroundColor: theme.iconBtn }]} onPress={openOutfitSelector}>
            <Text style={[styles.changeBtnText, { color: theme.primary }]}>Change Outfit</Text>
          </TouchableOpacity>
          <TouchableOpacity style={[styles.removeBtn, { backgroundColor: theme.background}]} onPress={removeLog}>
      ...
      <View style={[styles.emptyState, { backgroundColor: theme.card, borderColor: theme.border }]}>
        <Text style={[styles.emptyText, { color: theme.subtext }]}>You haven't logged an outfit for this day.</Text>
        <TouchableOpacity style={[styles.logButton, { backgroundColor: theme.primary }]} onPress={openOutfitSelector}>
  ...
  <Modal visible={isModalVisible} animationType="slide" transparent={true}>
    <View style={styles.modalOverlay}>
      <View style={[styles.modalContent, { backgroundColor: theme.card }]}>
        <Text style={[styles.modalTitle, { color: theme.text }]}>Select from Saved Outfits</Text>
        
        {availableOutfits.length === 0 ? (
          <Text style={[styles.noOutfitsText, { color: theme.subtext }]}>You don't have any saved outfits yet.</Text>
        ) : (
          <FlatList
            data={availableOutfits}
            keyExtractor={(item: OutfitItem) => item.id.toString()}
            renderItem={({ item }: { item: OutfitItem }) => (
              <TouchableOpacity style={[styles.outfitItem, { borderBottomColor: theme.border }]} onPress={() => logOutfit(item.id)}>
                <Text style={[styles.outfitItemName, { color: theme.text }]} numberOfLines={2}>{item.name}</Text>
                <Text style={[styles.selectText, { color: theme.primary }]}>Select</Text>
              </TouchableOpacity>
            )}
          />
        )}
        
        <TouchableOpacity style={[styles.closeBtn, { backgroundColor: theme.iconBtn }]} onPress={() => setModalVisible(false)}>
  ...
  <Modal visible={isJumpModalVisible} animationType="fade" transparent={true}>
    <View style={styles.modalOverlayJump}>
      <View style={[styles.jumpContent, { backgroundColor: theme.card }]}>
        <Text style={[styles.modalTitle, { color: theme.text }]}>Time Travel 🕰️</Text>
        <Text style={[styles.subtitleJump, { color: theme.subtext }]}>Enter the exact date you want to visit.</Text>
        
        <View style={styles.jumpInputsRow}>
          <TextInput placeholderTextColor={theme.subtext} style={[styles.jumpInput, { borderColor: theme.border, backgroundColor: theme.background, color: theme.text }]} placeholder="DD" keyboardType="numeric" maxLength={2} value={jumpDay} onChangeText={(t: string) => setJumpDay(t.replace(/[^0-9]/g, ''))} />
          <Text style={[styles.slash, { color: theme.subtext }]}>/</Text>
          <TextInput placeholderTextColor={theme.subtext} style={[styles.jumpInput, { borderColor: theme.border, backgroundColor: theme.background, color: theme.text }]} placeholder="MM" keyboardType="numeric" maxLength={2} value={jumpMonth} onChangeText={(t: string) => setJumpMonth(t.replace(/[^0-9]/g, ''))} />
          <Text style={[styles.slash, { color: theme.subtext }]}>/</Text>
          <TextInput placeholderTextColor={theme.subtext} style={[styles.jumpInputYear, { borderColor: theme.border, backgroundColor: theme.background, color: theme.text }]} placeholder="YYYY" keyboardType="numeric" maxLength={4} value={jumpYear} onChangeText={(t: string) => setJumpYear(t.replace(/[^0-9]/g, ''))} />
        </View>

        <View style={styles.actionsRow}>
          <TouchableOpacity style={[styles.cancelJumpBtn, { backgroundColor: theme.iconBtn }]} onPress={() => setJumpModalVisible(false)}>
            <Text style={[styles.cancelJumpText, { color: theme.text }]}>Cancel</Text>
          </TouchableOpacity>
          <TouchableOpacity style={[styles.confirmJumpBtn, { backgroundColor: theme.primary }]} onPress={handleJumpToDate}>
