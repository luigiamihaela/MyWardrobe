...
if (temp >= 25) return "Warm weather, perfect for a Tshirt!";
...
const [ootd, setOotd] = useState<{ name: string; images: string[] } | null>(null);

const [weatherData, setWeatherData] = useState<WeatherData | null>(null);
const [weatherLoading, setWeatherLoading] = useState(true);
const [weather, setWeather] = useState<{ temp: number, city: string, desc: string, icon: string } | null>(null); 
...
useFocusEffect(
    useCallback(() => {
      const randomQuote = STYLE_QUOTES[Math.floor(Math.random() * STYLE_QUOTES.length)];
      ...
      try {
        ...
        if (ootdRow) {
          const allImages = [
            ootdRow.dress_img, ootdRow.top_img, ootdRow.bottom_img, 
            ootdRow.shoes_img, ootdRow.outerwear_img, ootdRow.hat_img, ootdRow.purse_img
          ].filter(img => img != null);
...
<View style={[styles.container, { backgroundColor: theme.background }]}>
  ...
  <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
    ...
    {weatherLoading ? (
      <View style={[styles.weatherWidget, { backgroundColor: theme.iconBtn, justifyContent: 'center' }]}>
        <ActivityIndicator size="small" color={theme.primary} />
      </View>
    ) : weatherData ? (
      <View style={[styles.weatherWidget, { backgroundColor: theme.iconBtn }]}>
        <Ionicons name={getWeatherIcon(weatherData.weather[0].main) as any} size={36} color={theme.primary} />
    ...
    <Text style={[styles.sectionTitle, { color: theme.text }]}>Outfit of the Day</Text>
          ...
          {ootd ? (
            <View style={styles.ootdContentVertical}>
              <View style={styles.ootdHeader}>
                ...
                <Text style={[styles.ootdName, { color: theme.text }]}>{ootd.name}</Text>
              </View>
              
              <View style={styles.ootdGallery}>
                {ootd.images.length > 0 ? (
                  ootd.images.map((imgUri, index) => (
                    <Image key={index} source={{ uri: imgUri }} style={styles.ootdMiniItem} />
                  ))
                ) : (
                  <View style={[styles.ootdPlaceholderImage, { backgroundColor: theme.border }]}>
                    <Ionicons name="shirt-outline" size={30} color={theme.primary} />
                  </View>
                )}
              </View>
            </View>
            ...
            <View style={styles.ootdEmpty}>
              <Ionicons name="calendar-outline" size={40} color="#A0AEC0" style={{ marginBottom: 12 }} />
              <Text style={[styles.ootdEmptyTitle, { color: theme.text }]}>No outfit planned</Text>
              <Text style={styles.ootdEmptySub}>You haven't chosen what to wear today.</Text>
              <ThemedButton 
    ...
    <View style={styles.quoteContainer}>
      <Ionicons name="sparkles" size={20} color={theme.primary} style={{ marginBottom: 8 }} />
      <Text style={[styles.quoteText, { color: theme.subtext }]}>{quote}</Text>
    </View>
...
const styles = StyleSheet.create({
  ...
  themeToggle: { paddingHorizontal: 12, paddingVertical: 8, borderRadius: 20, justifyContent: 'center' },
  themeToggleText: { fontSize: 16, fontWeight: 'bold', color: '#1A202C' },
  scrollContent: { padding: 24 },
  
  statsRow: { flexDirection: 'row', justifyContent: 'space-between', marginBottom: 20, gap: 16 },
  statCard: { flex: 1, alignItems: 'center', paddingVertical: 20 },
  statNumber: { fontSize: 32, fontWeight: 'bold' },
  statLabel: { fontSize: 14, color: '#718096', fontWeight: '500', marginTop: 4 },
  ...
  weatherTextContainer: { marginLeft: 16, flex: 1 },
  weatherTitle: { fontSize: 18, fontWeight: 'bold', marginBottom: 2 },
  weatherSub: { fontSize: 13, fontWeight: '500' },
  
  sectionTitle: { fontSize: 20, fontWeight: 'bold', marginBottom: 16 },
  
  ootdCard: { padding: 0, overflow: 'hidden', marginBottom: 32 },
  ootdContentVertical: { padding: 20 },
  ootdHeader: { marginBottom: 16 },
  ootdBadge: { fontSize: 10, fontWeight: 'bold', color: '#FFFFFF', backgroundColor: '#48BB78', paddingHorizontal: 8, paddingVertical: 4, borderRadius: 8, alignSelf: 'flex-start', marginBottom: 8, overflow: 'hidden' },
  ootdName: { fontSize: 20, fontWeight: 'bold' },
  
  ootdGallery: { flexDirection: 'row', flexWrap: 'wrap', gap: 12 },
  ootdMiniItem: { width: 70, height: 70, borderRadius: 12, backgroundColor: '#EDF2F7', borderWidth: 1, borderColor: '#E2E8F0' },
  ootdPlaceholderImage: { width: 70, height: 70, borderRadius: 12, alignItems: 'center', justifyContent: 'center' },
  
  ootdEmpty: { alignItems: 'center', padding: 24 },
  ootdEmptyTitle: { fontSize: 18, fontWeight: 'bold', marginBottom: 4 },
  ootdEmptySub: { fontSize: 14, color: '#718096', marginBottom: 16, textAlign: 'center' },
  
  quoteContainer: { alignItems: 'center', paddingHorizontal: 20, paddingVertical: 10},
  quoteText: { fontSize: 15, fontStyle: 'italic', textAlign: 'center', lineHeight: 24, fontWeight: '500' },
});