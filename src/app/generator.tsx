...
export default function SmartGeneratorScreen() {
  ...
  const generateOutfit = () => {
    try {
      ...
      const seasonClothes =
        selectedSeason === "Any Season"
          ? allClothes
          : allClothes.filter(
              (item) =>
                item.season === selectedSeason || item.season === "All Year",
            );

      const tops = seasonClothes.filter((c) =>
        [1, 2, 3].includes(c.category_id),
      );
      const bottoms = seasonClothes.filter((c) =>
        [4, 5, 6].includes(c.category_id),
      );
      ...
      if (targetColor !== "Any Color") {
        const clothesWithTargetColor = seasonClothes.filter(
          (c) => c.color === targetColor,
        );
        if (clothesWithTargetColor.length === 0) {
          Alert.alert(
            "Missing Clothes",
            `You don't have any ${targetColor} clothes for ${selectedSeason}.`,
          );
          return;
        }
        ...
        if (anchorItem) {
          if ([1, 2, 3].includes(anchorItem.category_id))
            chosenTop = anchorItem;
          else if ([4, 5, 6].includes(anchorItem.category_id))
            chosenBottom = anchorItem;
      ...
      const canMakeTwoPiece =
        (tops.length > 0 || chosenTop !== null) &&
        (bottoms.length > 0 || chosenBottom !== null);
      const canMakeDress = dresses.length > 0 || chosenDress !== null;

      if (!canMakeTwoPiece && !canMakeDress) {
        Alert.alert(
          "Not Enough Clothes",
          `You need either a Top+Bottom or a Dress for ${selectedSeason}.`,
        );
        return;
      }
      if (shoes.length === 0 && !chosenShoes) {
        Alert.alert(
          "Missing Shoes",
          `You don't have any shoes for ${selectedSeason}!`,
        );
        return;
      }
      ...
      if (chosenDress) {
        ...
        buildDress =
          canMakeDress && (!canMakeTwoPiece || Math.random() > 0.5)
            ? true
            : false;
      }
      ...
      if (buildDress) {
        ...
        if (!chosenTop && !chosenBottom) {
          ...
          if (
            useColorTheory &&
            chosenTop &&
            !NEUTRAL_COLORS.includes(chosenTop.color)
          ) {
            compBottoms = bottoms.filter(
              (b) =>
                NEUTRAL_COLORS.includes(b.color) ||
                b.color === chosenTop!.color,
            );
          ...
          if (useColorTheory && !NEUTRAL_COLORS.includes(chosenBottom.color)) {
            compTops = tops.filter(
              (t) =>
                NEUTRAL_COLORS.includes(t.color) ||
                t.color === chosenBottom!.color,
            );
      ...
      if (!chosenOuterwear) {
        if (
          (selectedSeason === "Winter" || selectedSeason === "Autumn") &&
          outerwears.length > 0
        ) {
  ...
  const executeSave = (outfitName: string) => {
    ...
    try {
      ...
      Alert.alert(
        "Success!",
        `Outfit "${outfitName}" was saved to your collection.`,
      );
              ...
              <TouchableOpacity
                ...
                onPress={() => {
                  ...
                  const finalName =
                    ...
                    `Generated Outfit #${Date.now().toString().slice(-4)}`;
